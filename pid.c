...
/* variable ------------------------------------------------------------------- */
uint8_t VOFA_DebugData[sizeof(fp32_t) * 8];
 ...
 * 
 * @retval fp32_t The converted angle in radians.
 */
 ...
 * 
 * @retval fp32_t The calculated PID controller output value.
 */
...
{       
    ...
    fp32_t PID_Output = pidx.kp * pidx.Error + pidx.ki * pidx.IOutput + pidx.kd * pidx.DOutput;
 ...
 * 
 * @retval fp32_t The extracted 16-bit signed integer value.
 */
fp32_t DataExtraction(uint8_t data0, uint8_t data1)
{
    ...
    fp32_t fdata = (fp32_t)((data0 >> 7) ? -((nintdata >> 1)) : (uintdata));
    return fdata;
}
...
{
    fp32_t float_to_uint_data = roundf(data);
 ...
 * 
 * @retval fp32_t The adjusted actual angle value.
 */
fp32_t ZeroCrossing(fp32_t ExpectedAngle, fp32_t ActualAngle) {
    const fp32_t MAX_ANGLE = ROTOR_MECHANICAL_ANGLE_MAX;
    fp32_t DifferenceValue = ExpectedAngle - ActualAngle;
 ...
 * 
 * The function accepts 7 fp32_t parameters corresponding to 7 channels and 
 * stores them in the VOFA_DebugData array encapsulation, 
 * 7 fp32_t parameters can be set to EMPTY to indicate that they are not used,
...
{
    if (passage0 != EMPTY)
        *((fp32_t*)&VOFA_DebugData[sizeof(fp32_t) * 0]) = passage0;
    if (passage1 != EMPTY)
        *((fp32_t*)&VOFA_DebugData[sizeof(fp32_t) * 1]) = passage1;
    if (passage2 != EMPTY)
        *((fp32_t*)&VOFA_DebugData[sizeof(fp32_t) * 2]) = passage2;
    if (passage3 != EMPTY)
        *((fp32_t*)&VOFA_DebugData[sizeof(fp32_t) * 3]) = passage3;
    if (passage4 != EMPTY)
        *((fp32_t*)&VOFA_DebugData[sizeof(fp32_t) * 4]) = passage4;
    if (passage5 != EMPTY)
        *((fp32_t*)&VOFA_DebugData[sizeof(fp32_t) * 5]) = passage5;
    if (passage6 != EMPTY)
        *((fp32_t*)&VOFA_DebugData[sizeof(fp32_t) * 6]) = passage6;

    *((uint32_t*)&VOFA_DebugData[sizeof(fp32_t) * 7]) = 0x7f800000;
}
